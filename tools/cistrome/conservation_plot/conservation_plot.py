...
import tempfile
import gzip
from BasicStat.Func import * 
from Parser import WiggleIO, BedIO
...
def main():
    ...
    #quoted bug fix
    if options.phasdb and '"' in options.phasdb:
        options.phasdb = options.phasdb.replace('"','')
    ...
    if options.bedlabel and len(options.bedlabel) == bedfilenum:
        ...
        bedlable = [x.replace(' ', '_') for x in bedlabel ]
    ...
    for file_phasdb in files_phasdb:
        if file_phasdb.endswith('.wigFix'):
            name = file_phasdb.rstrip('.wigFix')
            phas_chrnames.append(name)

    if not phas_chrnames:
        error("%s has no valid phastcons db wigFix files!" % options.phasdb)
        sys.exit(1)
...
def extract_phastcons ( bedfile, phas_chrnames, width ):
    ...
    for chrom in chrs:
        ...
        #code above changed to that one
        wio = WiggleIO.WiggleIO(chrom + ".wigFix")
        wtrack = wio.build_wigtrack(bed)
        wtrack.sort()
...
def makeBmpFile(avgValues, wd, h,w, width, title, bedlabel):
    ...
    bmpname = bmpname.replace('\\', '/')
    #rscript = 'sink(file=file("/dev/null", "w"), type="message")\n'
    #rscript += 'sink(file=file("/dev/null", "w"), type="output")\n'    
    rscript = 'sink(file=NULL, "w"), type="message")\n'
    rscript += 'sink(file=NULL, "w"), type="output")\n'    
    rscript = 'bmp("%s",height=%d,width=%d)\n' %(bmpname,h,w)
