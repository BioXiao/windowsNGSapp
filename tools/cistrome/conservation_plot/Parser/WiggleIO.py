# Time-stamp: <2011-01-31 17:03:49 Tao Liu>
...
import re
import shutil
from FeatIO import WigTrackI
from BinKeeper import BinKeeperI
#from taolib.CoreLib.DB.BinKeeper import DBBinKeeperI
...
class WiggleIO:
    ...
    def build_wigtrack (self, bedO = None):
        ...
        for i in self.fhd:
            if i.startswith("track"):
                ...
                if pos_fixed:           # fixedStep
                    ...
                    add_val = False
                    if not bed_len == 0:
                        if bed_reg < bed_len and  pos_fixed >= pchrom[bed_reg][0]:
                            for cur_b in range(bed_reg, bed_len):
                                if pos_fixed in range(pchrom[cur_b][0],pchrom[cur_b][1]):
                                    add_val = True
                                    break
                                else:
                                    if pos_fixed < pchrom[cur_b][0]:
                                        break
                                    elif pos_fixed >= pchrom[cur_b][1]:
                                        bed_reg = cur_b + 1                                
                    else:
                        add_val = True
