...
def save_to_html_plain(output_dir, motifList, motifDists, distCutoff = 2.85):
    ...
    rscript.write('setwd("%s")\n' %(os.path.abspath(seqLogoFolder)).replace('\\', '/'))
    rscript.write('pkgTest <- function(x)\n')
    rscript.write('{if (!require(x,character.only = TRUE)){\n')
    rscript.write('  if (!require(x,character.only = TRUE))\n')
    rscript.write('  source("http://bioconductor.org/biocLite.R")\n')
    rscript.write('  biocLite(x)\n')
    rscript.write('  if(!require(x,character.only = TRUE)) stop("Package not found")}}\n')
    rscript.write('pkgTest("seqLogo")\n')
    for arg in args:
